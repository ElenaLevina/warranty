...
- ~~**openFile cleanup**~~ — добавлен `clearDecryptedCache()` (чистит `dec_*` в `cacheDir`),
  вызывается на старте в `sessionStore.bootstrap`. Пофайловую очистку после просмотра
  доработать при появлении экрана-просмотрщика.
